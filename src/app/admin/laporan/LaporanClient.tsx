...
export default function LaporanPage({ daftarTransaksi }: { daftarTransaksi: any[] }) {
  ...
  return (
    <div className="p-8 min-h-screen bg-[#0B1120]">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-white">Laporan Transaksi</h1>
          <p className="text-gray-400 mt-2">Filter dan ekspor data transaksi reservasi lapangan.</p>
        </div>
        <button onClick={handleExport} className="bg-[#2563EB] hover:bg-blue-600 text-white px-6 py-2.5 rounded-lg shadow-sm font-semibold transition-all hover:-translate-y-0.5">
          Export ke Excel
        </button>
      </div>

      <div className="bg-[#111827] p-6 rounded-xl shadow-sm border border-[#1F2937] mb-8 flex gap-4 items-end">
        <div>
          <label className="block text-sm font-semibold text-gray-300 mb-2">Bulan</label>
          <select value={bulan} onChange={(e) => setBulan(Number(e.target.value))} className="border border-[#374151] rounded-lg px-4 py-2 bg-[#1F2937] text-white focus:ring-2 focus:ring-[#3B82F6] focus:border-[#3B82F6] outline-none">
        ...
        <div>
          ...
          <input type="number" value={tahun} onChange={(e) => setTahun(Number(e.target.value))} className="border border-[#374151] rounded-lg px-4 py-2 bg-[#1F2937] text-white w-32 focus:ring-2 focus:ring-[#3B82F6] focus:border-[#3B82F6] outline-none" />
        </div>
      </div>

      <div className="bg-[#111827] rounded-xl shadow-sm border border-[#1F2937] overflow-hidden">
